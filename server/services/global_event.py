...
async def read_global_events_handler(db: AsyncSession) -> list[GlobalEvent]:
    stmt = select(GlobalEvent).options(selectinload(GlobalEvent.tags), selectinload(GlobalEvent.users), selectinload(GlobalEvent.venue), selectinload(GlobalEvent.organizer))
    result = await db.execute(stmt)
    return result.scalars().all()


async def add_global_event_handler(db: AsyncSession, title: str, description: str, date: str, organizer_id, venue: str,
                                   tickets: int, tags: list, users: list) -> GlobalEvent:
    uuid_id = str(uuid.uuid4())
    global_event = GlobalEvent(id=uuid_id, title=title, description=description, date=date, organizer_id=organizer_id,
                               venue=venue, tickets=tickets, tags=tags, users=users)
    db.add(global_event)
    await db.commit()
...
async def update_global_event_handler(db: AsyncSession, title: str, description: str, date: str, organizer_id,
                                      venue: str,
                                      tickets: int, tags: list, users: list) -> GlobalEvent | None:
...
async def delete_global_event_handler(db: AsyncSession, id: str) -> str | None:
    global_event = await db.execute(
        select(GlobalEvent).options(selectinload(GlobalEvent.tags),
